...
Verification for this workflows replays deterministic checks, validates typed schemas, compares package inventories to candidate S, and checks workflow results across Linux, Windows, and macOS. Independent review confirms gate boundaries, the comprehension result, audit ordering, traceability, and all nonwaivable gates. The current source is a candidate until the release verifier returns PASS and Core records the promotion; publisher signing and notarization are not claimed in V1.
